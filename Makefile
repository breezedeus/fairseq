# ein-gpu1
#WAV_DIR = /home/ein/jinlong/speech_data/holybell-sample
WAV_DIR = /Users/king/Documents/Ein/Codes/transfer-learning/data/audio/holybell-sample
OUT_DATA_DIR = data
PRETRAIN_MODEL_FP = pretrained/checkpoint_best.pt

gen-manifest:
	python examples/wav2vec/wav2vec_manifest.py $(WAV_DIR) --dest $(OUT_DATA_DIR) --ext wav --valid-percent 0.1

pretrain:
	python train.py $(OUT_DATA_DIR) --save-dir models --task audio_pretraining --criterion wav2vec --arch wav2vec2 \
		--num-workers 0 --encoder-layers 4 --update-freq 64 --sample-rate 8000 \
...
#	mv $(OUT_DATA_DIR)/valid-clean.tsv $(OUT_DATA_DIR)/valid.tsv
	python examples/wav2vec/build_vocab.py --base-vocab-fp $(OUT_DATA_DIR)/base_vocab.txt \
		--corpus-text-fp $(OUT_DATA_DIR)/train-text.txt -o $(OUT_DATA_DIR)/dict.ltr.txt

finetune-asr:
	python train.py $(OUT_DATA_DIR) --save-dir $(OUT_DATA_DIR)/finetuned-asr \
		--num-workers 0 --update-freq 24 --sample-rate 8000 \
		--w2v-path $(PRETRAIN_MODEL_FP) \
		--post-process letter --valid-subset valid --no-epoch-checkpoints --best-checkpoint-metric wer \
		--max-update 80000 --sentence-avg --task audio_pretraining --arch wav2vec_ctc --w2v-path \
